...
app.post("/api/admin/executives", requireAdminAuth,async (req, res) => {
  try {
    const {
      username,
      password,
...
app.get("/api/admin/executives", requireAdminAuth,async (req, res) => {
  try {
    const conn = await pool.getConnection();
    try {
      const [rows] = await conn.query(
...
app.put("/api/admin/executives/:id", requireAdminAuth,async (req, res) => {
  try {
...
// CREATE
app.post("/api/admin/operations", requireAdminAuth, async (req, res) => {
  try {
    const {
      name,
      username,
...
// UPDATE (optional)
app.put("/api/admin/operations/:id", requireAdminAuth, async (req, res) => {
  try {
    const id = Number(req.params.id);
    if (!id) return res.status(400).json({ message: "Invalid id." });
